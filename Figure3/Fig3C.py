...
def P_saccadeandpursuit(t): #position gating signal for saccade and pursuit variant
...
#run simulations for saccade and pursuit variant    
for c1 in cohs:
    if c1>0:
    ...
    if simulation == 'HeatmapData':
        ...
        leadersp[c1] = sols[:, :, 0]#np.mean(sols, axis = 0)[:, 0]
        followersp[c1] = sols[:, :, 1:10]#np.sum(np.mean(sols, axis=0)[:, 1:10], axis = 1)   
        followerunsumsp[c1] = sols[:, :, 1:10]#np.mean(sols, axis=0)[:, 1:10]

if simulation == 'HeatmapData':
    with open('FFresultsleadersp-full'+str(a)+'.pkl', 'wb') as handle:
        pkl.dump(leadersp, handle)

    with open('FFresultsfollowersp-full'+str(a)+'.pkl', 'wb') as handle:
        pkl.dump(followerunsumsp, handle)    
